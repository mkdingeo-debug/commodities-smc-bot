...
IMPORTANTE: Bitget confirmó oficialmente (comunicado del 10-feb-2026) que
ofrece 4 perpetuos de metales preciosos (oro, plata, platino, paladio),
además de los 3 energéticos (WTI, Brent, gas natural) y cobre que ya
confirma en su documentación — 8 materias primas en total, frente a las 3
que ofrece Bybit. El bot igual salta automáticamente cualquier símbolo que
no exista o falle puntualmente, sin afectar a los demás — revisa los Logs
de Render para confirmar cuáles llegan en cada corrida.
...
Variables de entorno:
    export SYMBOLS="XAUUSDT,XAGUSDT,XPTUSDT,XPDUSDT,COPPERUSDT,CLUSDT,BZUSDT,NATGASUSDT"
...
# Top de materias primas a probar en vivo. Bitget confirmó oficialmente
# (comunicado del 10-feb-2026) que ofrece 4 perpetuos de metales preciosos
# (oro, plata, platino, paladio) + los 3 energéticos ya confirmados en su
# documentación (WTI, Brent, gas natural) + cobre (confirmado con el
# símbolo COPPERUSDT, distinto del código de 3 letras XCU que usan otras
# casas). No encontramos evidencia de que Bitget tenga café, trigo u otros
# agrícolas como perpetuo, así que no se incluyen para no generar ruido de
# errores esperables. El bot igual salta automáticamente cualquier símbolo
# que no exista, por si Bitget suma más adelante.
DEFAULT_SYMBOLS = "XAUUSDT,XAGUSDT,XPTUSDT,XPDUSDT,COPPERUSDT,CLUSDT,BZUSDT,NATGASUSDT"
...
BOT_NAME = "materias_primas"  # debe coincidir con el check constraint de "bot" en la tabla senales
...
BOT_NAME_LEGIBLE = "Materias Primas"
...
def render_template_report(report: SignalReport, interval: str) -> str:
    ...
    L.append(f"═══ {label_prefix}RIDGECREST COMMODITIES | NATHANIEL RIDGE | {report.symbol} | TF {interval} ═══")
